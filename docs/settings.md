...
- **Preview.** Selecting a profile shows what it resolves to (models, agents, memory); the current profile's preview also ends with this session's account usage, the same report as `/usage`. Selecting never changes the session.
