...
import { projectProfileUsageSharing, type ProfileUsageSharing } from "../../profiles/usage";
...
export interface ProfileDashboardCallbacks {
	...
	importRoles(): void | Promise<void>;
	exportRoles(setup: ProfileDashboardSetupRef): void | Promise<void>;
...
function setupSidebarLabel(setup: ProfileDashboardSetupRef): string {
	...
	const name = setup.kind === "current" ? "Current setup" : cleanLine(setup.name);
	return `${emoji}${name}`;
}
...
export class ProfileDashboard implements Component {
	...
	#sharing: Record<string, ProfileUsageSharing[]> = {};
	#loadBlockReason: string | undefined;
	...
	constructor(options: ProfileDashboardOptions) {
		...
		this.#discoveryWarnings = [...(options.discoveryWarnings ?? [])].map(cleanLine).filter(Boolean);
		this.#loadBlockReason = options.loadBlockReason ? cleanLine(options.loadBlockReason) : undefined;
	}
	...
	#refreshUsageSharing(): void {
		const profiles: Array<{ profile: string; usage: ProfileSnapshot["usage"] }> = [];
		for (const state of this.#states.values()) {
			if (state.snapshot) profiles.push({ profile: state.snapshot.profile, usage: state.snapshot.usage });
		}
		this.#sharing = projectProfileUsageSharing(profiles);
	}

	setLoadBlockReason(reason: string | undefined): void {
		if (this.#disposed) return;
		this.#loadBlockReason = reason ? cleanLine(reason) : undefined;
		this.#callbacks.requestRender();
	}

	setDiscoveryWarnings(warnings: readonly string[]): void {
		if (this.#disposed) return;
		this.#discoveryWarnings = warnings.map(cleanLine).filter(Boolean);
		this.#callbacks.requestRender();
	}
	...
	render(width: number, allocatedHeight?: number, sidebarWidth?: number): readonly string[] {
		...
		if (this.#discoveryWarnings.length > 0) {
			const extra = this.#discoveryWarnings.length > 1 ? ` · +${this.#discoveryWarnings.length - 1} more` : "";
			notices.push(theme.fg("warning", `${theme.status.warning} Discovery: ${this.#discoveryWarnings[0]}${extra}`));
		}
	...
	handleInput(data: string): void {
		...
		if (data === "i") {
			void this.#callbacks.importRoles();
			return;
		}
		if (data === "e") {
		...
		if (data === "x") {
			const selected = this.selectedSetup;
			if (selected) void this.#callbacks.exportRoles(selected);
			return;
		}
	...
	#rebuildFilteredSetups(): void {
		...
		this.#filteredSetups = needle
			? this.#setups.filter(setup => {
					const label = setup.kind === "current" ? "Current setup" : setup.name;
	...
	#renderSetupList(width: number, height: number, startRow: number, startCol: number): string[] {
		...
		if (count === 0) {
			lines[0] = settingsTheme.hint("  No matching setups");
			this.#listScroll = 0;
	...
	#renderDetails(setup: ProfileDashboardSetupRef | undefined, width: number, height: number): string[] {
		...
		if (!setup) {
			...
			this.#detailScrollView?.dispose();
			this.#detailScrollView = undefined;
			return [theme.fg("muted", "No setup selected"), ...Array.from<string>({ length: height - 1 }).fill("")];
		}
		...
		const descriptor =
			setup.kind === "current"
				...
				: "Saved profile · read-only preview · current account pool";
		const status: string[] = [];
		...
		if (setup.kind === "saved" && this.#loadBlockReason) {
			status.push(theme.fg("warning", `${theme.status.warning} ${cleanLine(this.#loadBlockReason)}`));
		}
	...
	#footerHints(): FooterHint[] {
		...
		hints.push(
			...
			{ text: "i to import profile", action: "import-roles" },
			{ text: "x to export profile", action: "export-roles" },
		);
		...
		if (this.#focus === "profiles") {
			hints.push({ text: "↑/↓ to select setup" });
		} else {
	...
	#activateZone(action: HitZone["action"]): void {
		switch (action) {
			...
			case "export-roles": {
				const setup = this.selectedSetup;
				if (setup) void this.#callbacks.exportRoles(setup);
				break;
			}
		}
	}
