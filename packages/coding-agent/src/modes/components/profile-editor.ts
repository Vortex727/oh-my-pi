...
export class ProfileEditorComponent implements Component {
	...
	constructor(options: ProfileEditorOptions) {
		...
		for (const group of PROFILE_SETTINGS_GROUPS) {
			for (const path of getSetupGroupPaths(group.id)) eligible.add(path);
		}
		const fullHost = createSettingsHost({
			source: {
				get: path => {
					const configured = readConfigPath(this.#draft.config, path);
					return configured.present ? configured.value : this.#inheritedSettings.get(path);
				},
				set: (path, value) => writeConfigPath(this.#draft.config, path, structuredClone(value)),
			},
		});
		this.#settingsHost = {
			...fullHost,
	...
	#buildSections(): SettingsSelectorSection[] {
		...
		for (const group of PROFILE_SETTINGS_GROUPS) {
			...
			const paths = getSetupGroupPaths(group.id);
