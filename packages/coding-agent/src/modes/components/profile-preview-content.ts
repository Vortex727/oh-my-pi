...
import {
	formatProfileUsageRemaining,
	projectProfileUsage,
	type ProfileUsageLimitPresentation,
	type ProfileUsagePresentation,
	type ProfileUsageSharing,
} from "../../profiles/usage";
import type { ProfileDashboardSetupRef } from "./profile-dashboard";

const CREDENTIAL_BILLING_NOTICE = "Account limits; custom credentials may bill separately";
...
function plainProviderLabel(provider: string): string {
	return cleanLine(provider)
		.split(/[-_]/g)
		.filter(Boolean)
...
function quotaBar(
	...
	status?: ProfileUsagePresentation["providers"][number]["limits"][number]["status"],
): string {
	if (remainingPercent === undefined || !Number.isFinite(remainingPercent)) return "";
	const clamped = Math.min(100, Math.max(0, remainingPercent));
	const filled = Math.round(clamped / 10);
	const color =
		status === "exhausted" || clamped === 0 ? "error" : status === "warning" || clamped <= 20 ? "warning" : "success";
	return `${theme.fg(color, theme.progress.filled.repeat(filled))}${theme.fg("dim", theme.progress.empty.repeat(10 - filled))}`;
}

function providerSummary(provider: ProfileUsagePresentation["providers"][number]): string {
	let summary: string;
	if (provider.state === "usage-unavailable") {
		summary = "Usage unavailable";
	} else if (provider.state === "no-authenticated-account") {
		summary = "No authenticated account";
	} else {
		summary = "Quota not reported";
	}
	return summary;
}

interface UsageRowLayout {
	labelWidth: number;
	remainingWidth: number;
	stacked: boolean;
}

function sharedWindowLabel(window: ProfileUsagePresentation["providers"][number]["sharedWindows"][number]): string {
	if (!window.meter) return window.window;
	return `${window.window} (${window.meter.charAt(0).toUpperCase()}${window.meter.slice(1)})`;
}

function detailedLimits(
	provider: ProfileUsagePresentation["providers"][number],
): readonly ProfileUsageLimitPresentation[] {
	return provider.limits.filter(limit => !limit.shared || limit.remainingPercent === undefined);
}

function usageRowLayout(presentation: ProfileUsagePresentation, width: number): UsageRowLayout {
	const labels: string[] = [];
	const remaining: string[] = [];
	for (const provider of presentation.providers) {
		for (const window of provider.sharedWindows) {
			labels.push(`${cleanLine(sharedWindowLabel(window))}:`);
			if (window.remainingPercent !== undefined) {
				remaining.push(`${window.remainingPercent.toFixed(0)}% left`);
			}
		}
		for (const limit of detailedLimits(provider)) {
			labels.push(`${cleanLine(limit.label)}:`);
			if (limit.remainingPercent !== undefined) remaining.push(formatProfileUsageRemaining(limit));
		}
	}
	const available = width - 4 - 4 - 10;
	if (available < 16) return { labelWidth: 1, remainingWidth: 1, stacked: true };
	const labelNatural = maxCellWidth(labels, 8, available - 8);
	const remainingNatural = maxCellWidth(remaining, 8, available - 8);
	if (labelNatural + remainingNatural <= available) {
		return { labelWidth: labelNatural, remainingWidth: remainingNatural, stacked: false };
	}
	const remainingWidth = Math.max(8, Math.min(remainingNatural, Math.floor(available * 0.48)));
	return {
		labelWidth: Math.max(8, available - remainingWidth),
		remainingWidth,
		stacked: false,
	};
}

function renderUsageMetricRow(
	lines: string[],
	label: string,
	remaining: string,
	bar: string,
	layout: UsageRowLayout,
	width: number,
): void {
	const renderedLabel = theme.fg("muted", `${cleanLine(label)}:`);
	if (layout.stacked) {
		pushIndented(lines, renderedLabel, 4, width);
		pushIndented(lines, `${remaining}${bar ? `  ${bar}` : ""}`, 6, width);
		return;
	}
	if (!bar) {
		lines.push(
			...renderWrappedTableRow(
				[{ text: renderedLabel }, { text: remaining }],
				[layout.labelWidth, layout.remainingWidth + 12],
				width,
				{ indent: "    ", gap: "  " },
			),
		);
	...
	lines.push(
		...renderWrappedTableRow(
			[{ text: renderedLabel }, { text: remaining }, { text: bar }],
			[layout.labelWidth, layout.remainingWidth, 10],
			width,
			{ indent: "    ", gap: "  " },
		),
	);
...
function renderUsageLimit(
	lines: string[],
	limit: ProfileUsageLimitPresentation,
	now: number,
	layout: UsageRowLayout,
	width: number,
): void {
	const applicability = limit.shared ? "Absolute" : limit.applicability === "model" ? "Model-scoped" : "Scoped";
	const scope = [applicability, limit.accountLabel, limit.modelId, limit.tier, limit.windowLabel ?? limit.windowId]
		.filter(Boolean)
		.map(cleanLine)
		.join(" · ");
	const reset =
		limit.resetsAt && limit.resetsAt > now
			...
			: cleanLine(limit.resetLabel);
	const stale = limit.stale ? "cached/stale" : "";
	const overage =
		limit.usedFraction !== undefined && limit.usedFraction > 1
			? `${Math.round((limit.usedFraction - 1) * 100)}% over limit`
			: limit.status === "exhausted"
				? "Exhausted"
				: "";
	const bar = quotaBar(limit.remainingPercent, limit.status);
	renderUsageMetricRow(lines, limit.label, formatProfileUsageRemaining(limit), bar, layout, width);
	const detail = [scope, overage, reset, stale].filter(Boolean).join(" · ");
	if (detail) {
		pushIndented(lines, limit.stale ? theme.fg("warning", detail) : theme.fg("dim", detail), 6, width);
	}
	for (const note of limit.notes) pushIndented(lines, theme.fg("dim", cleanLine(note)), 6, width);
}

function renderProfileSharing(lines: string[], sharing: readonly ProfileUsageSharing[], width: number): void {
	for (const item of sharing) {
		const profiles = item.profiles.map(cleanLine).filter(Boolean).join(", ");
		const relation =
			item.kind === "confirmed"
				? `Confirmed · Shared with ${profiles || "another profile"}`
				: item.kind === "possible"
					? `Possible · Possibly shared with ${profiles || "another profile"}`
					: profiles
						? `Unknown · Sharing with ${profiles} is unconfirmed`
						: "Unknown · Other-profile sharing not confirmed";
		const account = cleanLine(item.accountLabel);
		pushIndented(
			lines,
			`${theme.fg(item.kind === "confirmed" ? "success" : item.kind === "possible" ? "warning" : "muted", relation)}${account ? theme.fg("dim", ` · ${account}`) : ""}`,
			4,
			width,
		);
	}
...
function renderProviderBlock(
	provider: ProfileUsagePresentation["providers"][number],
	sharing: readonly ProfileUsageSharing[],
	now: number,
	layout: UsageRowLayout,
	width: number,
): string[] {
	const lines: string[] = [];
	const stale = provider.stale ? theme.fg("warning", " · cached/stale") : "";
	pushIndented(lines, `${theme.bold(cleanLine(provider.providerLabel))}${stale}`, 2, width);
	if (
		provider.sharedWindows.length === 0 &&
		provider.limits.length === 0 &&
		provider.accountsWithoutUsage.length === 0
	) {
		pushIndented(lines, theme.fg("dim", providerSummary(provider)), 4, width);
	}
	for (const window of provider.sharedWindows) {
		const remaining =
			window.remainingPercent === undefined
				? `${window.remainingAccounts.toFixed(1)}/${window.accounts} account-equivalent left`
			...
			window.remainingPercent === undefined
				...
				: window.remainingPercent <= 0
					...
					: window.remainingPercent <= 20
						? "warning"
						: "success";
		renderUsageMetricRow(
			lines,
			sharedWindowLabel(window),
			theme.fg(color, remaining),
			quotaBar(window.remainingPercent),
			layout,
			width,
		);
		const resets = window.resetsAt.filter(reset => reset > now).map(reset => formatDuration(reset - now));
		if (resets.length > 0) {
			pushIndented(lines, theme.fg("dim", `resets in ${resets.join(", ")}`), 6, width);
		}
	}
	if (provider.scopedCapExhausted) {
		pushIndented(
			lines,
			theme.fg("error", `${theme.status.error} Scoped cap exhausted · shared headroom unchanged`),
			4,
			width,
		);
	}
	const details = detailedLimits(provider);
	if (details.length > 0) pushIndented(lines, theme.fg("dim", "Scoped / absolute limits"), 4, width);
	for (const limit of details) renderUsageLimit(lines, limit, now, layout, width);

	const summarizedSharedLimits = provider.limits.filter(limit => limit.shared && !details.includes(limit));
	const sharedAccounts = [
		...new Set(summarizedSharedLimits.map(limit => cleanLine(limit.accountLabel)).filter(Boolean)),
	];
	if (sharedAccounts.length > 0) {
		pushIndented(lines, theme.fg("dim", `Accounts · ${sharedAccounts.join(", ")}`), 4, width);
	}
	for (const limit of summarizedSharedLimits) {
		for (const note of limit.notes) {
			pushIndented(lines, theme.fg("dim", `${cleanLine(limit.label)} · ${cleanLine(note)}`), 6, width);
		}
	}
	if (provider.accountsWithoutUsage.length > 0) {
		pushIndented(
			lines,
			theme.fg(
				"warning",
				`${theme.status.warning} ${provider.accountsWithoutUsage.length} account(s) without usage reports`,
			),
			4,
			width,
		);
		for (const account of provider.accountsWithoutUsage) {
			const accountType = account.type === "api_key" ? "API key" : "OAuth account";
			pushIndented(lines, theme.fg("dim", `${accountType} · ${cleanLine(account.label)}`), 6, width);
		}
	}
	for (const note of provider.notes) pushIndented(lines, theme.fg("dim", cleanLine(note)), 4, width);
	renderProfileSharing(lines, sharing, width);
	return wrapLines(lines, width);
}

function assignedProviders(snapshot: ProfileSnapshot): string[] {
	const providers = new Set<string>(Object.keys(snapshot.credentialSources).map(cleanLine).filter(Boolean));
	for (const role of snapshot.roles) if (role.provider) providers.add(cleanLine(role.provider));
	for (const agent of snapshot.agents) if (agent.provider) providers.add(cleanLine(agent.provider));
	return [...providers].filter(Boolean).sort((left, right) => left.localeCompare(right));
}
...
function renderProviderGrid(
	...
	const lines: string[] = [];
	for (let blockIndex = 0; blockIndex < blocks.length; blockIndex += columns) {
		if (blockIndex > 0) lines.push("");
		const row = blocks.slice(blockIndex, blockIndex + columns);
		const rowCount = Math.max(0, ...row.map(block => block.length));
		for (let rowIndex = 0; rowIndex < rowCount; rowIndex++) {
			let line = "";
			for (const [columnIndex, block] of row.entries()) {
				const content = block[rowIndex] ?? "";
				line += content;
				if (columnIndex < row.length - 1) {
					line += `${padding(Math.max(0, cellWidth - visibleWidth(content)))}${separator}`;
				}
			}
			lines.push(line);
		}
...
function renderUsage(
	...
	sharingByProfile: Readonly<Record<string, ProfileUsageSharing[]>>,
	width: number,
): string[] {
	const now = Date.now();
	const presentation = projectProfileUsage(snapshot.usage, now);
	const lines: string[] = [theme.fg("dim", `${theme.status.info} ${CREDENTIAL_BILLING_NOTICE}`)];
	const gapWidth = 3;
	const minimumCellWidth = 46;
	const columns = Math.max(1, Math.min(3, Math.floor((width + gapWidth) / (minimumCellWidth + gapWidth))));
	const cellWidth = Math.floor((width - gapWidth * (columns - 1)) / columns);
	const layout = usageRowLayout(presentation, cellWidth);
	const assigned = assignedProviders(snapshot);
	const assignedKeys = new Set(assigned.map(provider => provider.toLocaleLowerCase()));
	const sharing = sharingByProfile[snapshot.profile] ?? [];
	const sharingByProvider = new Map<string, ProfileUsageSharing[]>();
	for (const item of sharing) {
		const key = cleanLine(item.provider).toLocaleLowerCase();
		if (!key) continue;
		const rows = sharingByProvider.get(key) ?? [];
		rows.push(item);
		sharingByProvider.set(key, rows);
	}

	const presentationByProvider = new Map(
		presentation.providers.map(provider => [cleanLine(provider.provider).toLocaleLowerCase(), provider] as const),
	);
	const providerLabels = new Map<string, string>();
	const orderedKeys: string[] = [];
	const addProvider = (provider: string): void => {
		const cleaned = cleanLine(provider);
		const key = cleaned.toLocaleLowerCase();
		if (!key || providerLabels.has(key)) return;
		providerLabels.set(key, cleaned);
		orderedKeys.push(key);
	};
	for (const provider of presentation.providers) addProvider(provider.provider);
	for (const provider of assigned) addProvider(provider);
	for (const item of sharing) addProvider(item.provider);

	const blocks: string[][] = [];
	for (const key of orderedKeys) {
		const provider = presentationByProvider.get(key);
		const providerSharing = sharingByProvider.get(key) ?? [];
		if (provider) {
			blocks.push(renderProviderBlock(provider, providerSharing, now, layout, cellWidth));
			continue;
		}
		const block: string[] = [];
		const providerLabel = providerLabels.get(key) ?? key;
		pushIndented(block, theme.bold(plainProviderLabel(providerLabel)), 2, cellWidth);
		if (assignedKeys.has(key)) pushIndented(block, theme.fg("dim", "Quota not reported"), 4, cellWidth);
		renderProfileSharing(block, providerSharing, cellWidth);
		blocks.push(wrapLines(block, cellWidth));
	}
	if (blocks.length === 0) {
		lines.push(theme.fg("muted", "  Usage unavailable"));
	} else {
		lines.push(...renderProviderGrid(blocks, width, columns, cellWidth));
	}
	return wrapLines(lines, width);
}
...
export function buildProfilePreviewOverview(options: {
	...
	snapshot: ProfileSnapshot;
	sharing: Readonly<Record<string, ProfileUsageSharing[]>>;
	width: number;
}): string[] {
	const { setup, snapshot, sharing } = options;
	...
	lines.push("", sectionHeading("Usage & limits"), ...renderUsage(snapshot, sharing, width));
	return wrapLines(lines, width);
}
