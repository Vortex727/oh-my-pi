...
function assertReady(options: ApplySetupModelRolesOptions): void {
	if (options.signal?.aborted) throw new DOMException("Setup application cancelled", "AbortError");
...
function assertRoleName(role: string): void {
	if (!role.trim() || role === "__proto__" || role === "constructor" || role === "prototype") {
		throw new Error("Setup contains an invalid model role name");
	}
}

function unresolvedRoleError(role: string): Error {
	return new Error(
		`Model role ${JSON.stringify(role)} is not available in the current session. Choose an available model or load the setup in a fresh session.`,
	);
}

function invalidRoleError(role: string): Error {
	return new Error(
		`Model role ${JSON.stringify(role)} has an invalid selector. Correct the setup or load it in a fresh session.`,
	);
}
...
function assertContextFits(options: ApplySetupModelRolesOptions, target: Model): void {
	...
	if (!context) return;
	const reserve = resolveBudgetReserveTokens(contextWindow, options.settings.getGroup("compaction"));
	if (context.tokens <= Math.max(0, contextWindow - reserve)) return;
	throw new Error(
		`The current conversation does not fit ${target.provider}/${target.id}. Compact this conversation first, or load the setup in a fresh session.`,
	);
}
...
function applicationFailure(error: unknown, restoreError: unknown, session: AgentSession): AggregateError {
	...
	return new AggregateError(
		[error, restoreError],
		`Setup application failed and the previous model state could not be restored. The active model is ${active}.`,
	);
}

/** Apply only a saved setup's model-role overlay to the live conversation. */
export async function applySetupModelRoles(options: ApplySetupModelRolesOptions): Promise<void> {
	...
	if (!previousModel) {
		throw new Error("The current session has no active model. Load the setup in a fresh session.");
	}
	...
	try {
		...
		assertReady(options);
		options.settings.overrideModelRoles(options.roles);
	} catch (error) {
		const stateChanged =
			sessionMutated ||
