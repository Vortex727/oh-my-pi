...
function projectSetting(settingPath: SettingPath, settings: Settings): ProfileSettingRow {
	...
	const displayable =
		...
		((definition.type === "boolean" && typeof value === "boolean") ||
			...
			(definition.type === "enum" && typeof value === "string"));
	return {
		path: settingPath,
		label: ui?.label ?? settingPath,
		value: displayable ? (value as boolean | number | string) : null,
		hidden: !displayable,
		configured: settings.isConfigured(settingPath),
	};
}
...
export async function buildProfileSnapshot(options: BuildProfileSnapshotOptions): Promise<ProfileSnapshot> {
	...
	return {
		...
		settings: (Object.keys(SETTINGS_SCHEMA) as SettingPath[]).map(settingPath =>
			projectSetting(settingPath, settings),
		),
		warnings: [],
	};
}
