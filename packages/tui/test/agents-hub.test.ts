...
class TestSettings {
	...
	setListMember(key: string, item: string, member: boolean): void {
		const rest = (this.lists.get(key) ?? []).filter(entry => entry !== item);
		this.lists.set(key, member ? [...rest, item] : rest);
		this.writes.push(`${key}[${item}]`);
	}

	setRecordEntry(key: string, name: string, value: string | undefined): void {
...
async function createHub(
	...
	const hub = await AgentsHubComponent.create(
		tuiStub,
		{
			...
			setAgentOverride: (property, name, value) => settings.setRecordEntry(OVERRIDE_KEYS[property], name, value),
